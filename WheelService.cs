...
public sealed class WheelService
{
    ...
    /// Выполняет розыгрыш.
    /// </summary>
    /// <param name="allPlayers">Полный список активных сотрудников.</param>
    /// <param name="participantIds">ID тех, кто участвует в текущей смене.</param>
    /// <returns>Объект победителя или null, если участников нет.</returns>
    public Player? Spin(List<Player> allPlayers, List<int> participantIds)
    {
        if (participantIds is null || participantIds.Count == 0) return null;
        ...
        // --- Шаг 4: сумма коэффициентов участников ---
        int S = participants.Sum(p => p.Coefficient);
